...
class OwnerController {
    ...
    public String saveOwner(Owners owner) {
        //Prevents requests from overwriting other users. Previously, if the request provided an ID that was already in use, the original user would be overwritten.
        if (!ownersRepository.findById(owner.getId()).equals(Optional.empty())) {
            return "ID already belongs to another user. Please choose a new ID.";
        }
    ...
    public Optional<Owners> findOwnerById(@PathVariable("ownerId") int id) {
        logger.info("Entered into Controller Layer");
        //Creates a dummy owner if the requested ID does not have an owner already attached.
        if (ownersRepository.findById(id).equals(Optional.empty())) {
            Owners o1 = new Owners(1, "No owner exists under the given ID. Dummy owner created.", "placeholder", "placeholder", "placeholder");
            ownersRepository.save(o1);
            return ownersRepository.findById(1);
        }
